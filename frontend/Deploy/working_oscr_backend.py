...
class WorkingOSCR:
    ...
    def isolate_combats(self, path: str, max_combats: int = -1):
        """Echte Combat-Isolation mit korrekten Patterns"""
        ...
        try:
            ...
            # Suche nach Combat-Patterns - basierend auf echten OSCR-Patterns
            ...
            for i, line in enumerate(lines):
                if '::' in line:
                    ...
                    # Combat-Patterns erkennen
                    is_combat_line = False
                    ...
                    if is_combat_line:
                        if not in_combat:
                            ...
                            current_combat_lines = 0
                            last_combat_time = time_part
                        
                        current_combat_lines += 1
                        
                        # Combat beenden wenn genug Zeilen gefunden
                        if current_combat_lines >= self._settings['combat_min_lines']:
                            # Zeit-Extraktion
                            try:
                                # Zeit-Format: 25:10:04:14:11:41.1
                                time_parts = time_part.split(':')
                                if len(time_parts) >= 6:
                                    date_str = f"2025-{time_parts[1]}-{time_parts[2]}"
                        ...
                        # Nicht-Combat-Zeile - Combat beenden wenn genug Zeilen
                        if in_combat and current_combat_lines >= self._settings['combat_min_lines']:
                            # Letzten Combat hinzufügen
                            try:
                                time_parts = last_combat_time.split(':')
                                if len(time_parts) >= 6:
                                    date_str = f"2025-{time_parts[1]}-{time_parts[2]}"
                                    time_str = f"{time_parts[3]}:{time_parts[4]}:{time_parts[5]}"
                                else:
