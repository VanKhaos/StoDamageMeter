"""
Echte OSCR-API für WPF-Integration
Verwendet die vollständige OSCR-Funktionalität ohne Mock-Daten
"""
...
try:
    ...
    logger.info("Imported OSCR modules as package.")
except ImportError:
    ...
    try:
        ...
        # Direkte Imports ohne OSCR-Präfix
        from main import OSCR
        from datamodels import OverviewTableRow, Combat, CritterMeta
        logger.info("Successfully imported OSCR modules directly.")
    except ImportError as e:
        logger.error(f"Critical: Could not import OSCR modules: {e}")
        sys.exit(1)
...
def get_health_status() -> Dict[str, Any]:
    ...
    try:
        # Versuche, eine Instanz von OSCR zu erstellen, um die Verfügbarkeit zu prüfen
        parser = OSCR()
