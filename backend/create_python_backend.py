#!/usr/bin/env python3
"""
Erstellt ein Python-Backend-Script für die WPF-Anwendung
"""
...
def create_python_backend():
    """Erstellt ein Python-Backend-Script"""
    print("Creating Python backend script...")
    
    # Python-Backend-Script erstellen
    python_backend = '''#!/usr/bin/env python3
"""
...
'''
    ...
    # Schreibe das Python-Backend-Script
    with open('python_backend.py', 'w', encoding='utf-8') as f:
        f.write(python_backend)
    
    # Kopiere OSCR-Verzeichnis
    if Path('OSCR').exists():
        if Path('python_backend_OSCR').exists():
            shutil.rmtree('python_backend_OSCR')
        shutil.copytree('OSCR', 'python_backend_OSCR')
        print("PASS OSCR directory copied")
    else:
        print("FAIL OSCR directory not found")
        return False
    
    print("PASS Python backend script created")
    return True
